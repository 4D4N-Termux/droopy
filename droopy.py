...
def main():
    ...
    if args:
        print("Файл конфигурации найден в: {0}".format(cfg))
    ...
    if args['save_config']:
        ...
        save_options(cfg)
        print("Опции сохранены в {0}".format(cfg))
    print("Файлы были сохранены в {0}\n".format(args['directory']))
    proto = 'https' if args['ssl'] else 'http'
    print("HTTP сервер запущен...",
          "Проверьте адресс: {0}://localhost:{1}".format(proto, args['port']))
    try:
        run(port=args['port'],
            certfile=args['ssl'],
